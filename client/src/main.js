...
function normalizedFileType(file) { if (!file) return ""; const extension = file.name?.split(".").pop()?.toLowerCase(); const extensionType = { mp4: "video/mp4", m4v: "video/mp4", webm: "video/webm", ogv: "video/ogg", ogg: "video/ogg", "3gp": "video/3gpp", "3gpp": "video/3gpp" }[extension]; return file.type?.startsWith("video/") ? file.type : extensionType || file.type || ""; }
function videoType(item) { if (item.mime_type?.startsWith("video/")) return item.mime_type; const extension = item.file_path?.split(".").pop()?.toLowerCase(); return { mp4: "video/mp4", m4v: "video/mp4", webm: "video/webm", ogv: "video/ogg", ogg: "video/ogg", "3gp": "video/3gpp", "3gpp": "video/3gpp" }[extension] || "video/mp4"; }
...
function card(item, index) {
  const media = item.type === "photo" ? (item.image ? `<img src="${item.image}" alt="${item.title}"/>` : `<div class="photo-placeholder">imagem indisponível</div>`) : item.type === "video" ? (item.image ? `<video controls preload="metadata"><source src="${item.image}" type="${videoType(item)}"/>Seu navegador não consegue reproduzir este vídeo.</video>` : `<div class="photo-placeholder">vídeo indisponível</div>`) : item.type === "message" && item.mime_type?.startsWith("audio/") ? (item.image ? `<audio src="${item.image}" controls></audio>` : `<div class="photo-placeholder">áudio indisponível</div>`) : "";
...
async function handleMemory(event) {
  ...
  try {
    if (file?.size) {
      ...
      if (!["photo", "video", "message"].includes(type) || !["image/", "video/", "audio/"].some((kind) => fileType.startsWith(kind))) throw new Error("Escolha um arquivo compatível com o tipo selecionado.");
      ...
      const upload = await withTimeout(supabase.storage.from("memories").upload(uploadedPath, prepared, { contentType: type === "photo" ? "image/jpeg" : fileType, cacheControl: "3600", upsert: false }), 60000, "O envio demorou demais. Verifique sua conexão e tente novamente.");
      if (upload.error) throw new Error(upload.error.message);
      filePath = uploadedPath;
    }
    const selectedDate = data.get("date") || await detectPhotoDate(file) || new Date().toISOString().slice(0, 10);
    const payload = { title: data.get("title"), note: data.get("note") || null, date: selectedDate, file_path: filePath, mime_type: file ? normalizedFileType(file) : state.editing?.mime_type || null, source_url: data.get("source_url") || null };
